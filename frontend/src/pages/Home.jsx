  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex flex-col relative overflow-hidden">
       <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute bottom-10 right-10 w-62 h-62 bg-blue-300 opacity-20 rounded-full animate-pulseSlow"></div>
      </div>
      {/* Main Content */}
      ...
      <div className="w-full bg-blue-50  py-3 px-4 -mt-[35px] relative z-20">
        <h2 className="text-4xl font-bold text-center text-blue-800 mb-12 animate-bounceSlow">
          Our Contributors
        </h2>
        <div className="flex flex-wrap justify-center gap-12">
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className="flex flex-col items-center bg-white shadow-md rounded-xl p-0 w-64  hover:shadow-lg transition"
            >
              <div className="w-40 h-40 rounded-full overflow-hidden flex items-center justify-center mt-3 mb-4 border-4 border-blue-100 bg-white">
                <img
                  src={member.image}
                  alt={member.name}
